...
function changeText(newText){
    document.getElementById("text-area").value=newText.replace(/^\n/,"");
}
//accepts the index 
//and changes the textbox with that index
function animate(idx){
    if(state.isAnimating){
        changeText(state.animation[idx]);
        setTimeout(function(){
            animate((idx+1)%state.animation.length);
        },state.isTurbo?50:250);
        console.log(state);
    }
}
